import {
  GameLogic,
  GameModel
} from "@swg-common/../../common/src/game/gameLogic";
...
export async function voteHandler(
  event: Event<VoteRequestBody>
): Promise<HttpResponse<VoteResponse>> {
  ...
  try {
    ...
    let totalVotes = await redisManager.get<number>(
      `user-${user.id}-${generation}-votes`,
      0
    );
    ...
    const voteHexes = await redisManager.getString(
      `user-${user.id}-${generation}-vote-hex`,
      ""
    );
