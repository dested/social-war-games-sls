export class AnimationUtils {
    ...
    static stopAnimations() {
        for (let i = 0; i < AnimationUtils.animations.length; i++) {
            let animation = AnimationUtils.animations[i];
            animation.stop = true;
        }
    ...
    static start(options: {
        ...
        let animationInstance = new AnimationInstance();
        ...
        function next() {
            ...
            let percent = Math.max(Math.min((curTime - startTime) / options.duration, 1), 0);
            let j = options.easing(percent);
    ...
    static lightenDarkenColor(col: string, amount: number) {
        ...
        let num = (parseInt as any)(col, 16);
        let r = (num >> 16) + amount;
